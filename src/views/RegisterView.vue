              ...
              <div class="card">
                <div class="card-body py-5 px-md-5">
                  <form @submit.prevent="submitForm">
                    ...
                    <button type="submit" class="btn btn-primary btn-block mb-4">{{ otpSent ? 'Verify OTP' : 'Register' }}</button>
...
async function submitForm() {
  try {
    ...
    if (!otpSent.value) {
      const response = await axios.post('https://103.175.218.179:8181/register', {
      ...
      const response = await axios.post('https://103.175.218.179:8181/verify_register', {
        username: username.value,
        otp: otp.value,
      });
